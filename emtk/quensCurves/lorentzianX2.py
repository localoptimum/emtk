"""Two lorentzian curves with different widths
...
A fit of the data attempts to identify gamma, the line width.  A least
squares fit would also determine the amplitude of the curve,
proportional to the number density and contrast.  These can be
computed numerically knowing the total number of neutrons counted and
the gamma parameter, and the uncertainty on that parameter likewise
from the variance on gamma.
...
class DoubleLorentzian(base.Curve):
    ...
    def setup_guesses(self):
        ...
        """
        dmean = np.mean(self.data)
        self.guesses = np.array([dmean * 0.8, dmean * 1.2])
    ...
    def curve(self, params, dat=None):
        ...
        lor1 = (k1 / np.pi) * 1.0 / (xx**2.0 + k1**2.0)
        lor2 = (k2 / np.pi) * 1.0 / (xx**2.0 + k2**2.0)
        return lor1 + lor2
    ...
    def cdf(self, params, x):
        ...
        cdf = 0.5*(1.0/np.pi) * (np.pi + np.arctan( x / k1 ) + np.arctan( x / k2))
        return cdf
    ...
    def report(self):
        print("Double lorentzian curve maximum likelihood estimation")
